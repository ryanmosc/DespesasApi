...
public class ComprovanteService {
    ...
    public void salvarComprovante(MultipartFile file, Long idDespesa){
        ...
        Despesa despesa = despesaRepository.findById(idDespesa)
                .orElseThrow(() -> new DespesaNaoEncontradaException("Nenhuma despesa encontrada."));

        if (!despesa.getUser().getId().equals(userId)){
            throw new AcessoNegadoException("Usuário não possui acesso a esta despesa.");
        }
    ...
    public ResponseEntity<byte[]> visualizarComprovante(Long idComprovante){
        ...
        Comprovante comprovante = comprovanteRepository.findById(idComprovante)
                .orElseThrow(() -> new ComprovanteNaoEncontradoException("Nenhum comprovante localizado com este id."));

        Long userId = getUserById.getUserById().getId();

        if (!comprovante.getDespesa().getUser().getId().equals(userId)){
            throw new AcessoNegadoException("Usuário não possui acesso a este comprovante.");
        }
        ...
        String nomeArquivo = comprovante.getFileName() != null
    ...
    public void deletarComprovante(Long id){
        ...
        if (!comprovanteRepository.existsById(id)){
            throw new ComprovanteNaoEncontradoException("Comprovante não existe.");
        }
